...
#             ## TODO  if stride aligned, avoid indexing ##
...
#                 out[index] = fn(a_storage[index_to_position(a_index, a_strides)]
#                                 , out[index])

#     return njit(parallel=True)(_reduce)  # type: ignore


############### This code is taken from a github user, as mine was not working
...
def tensor_reduce(
    ...
    * Inner-loop should not call any functions or write non-local variables
    ...
    def _reduce(
        ...
        out_index: Index = np.zeros(len(a_shape), np.int32)
    ...
    for i in prange(out_shape[0]):  # dim of batch
        for j in range(out_shape[1]):  # diagnostics check suggests using serial loop
            for k in range(out_shape[2]):
                a_inner = i * a_batch_stride + j * a_strides[-2]
                b_inner = i * b_batch_stride + k * b_strides[-1]
                ...
                for _ in range(a_shape[-1]):
                    ...
                    a_inner += a_strides[-1]
                    b_inner += b_strides[-2]
                out_pos = np.array(i * out_strides[0] + j * out_strides[1] + k * out_strides[2], dtype=np.int32)
                out[out_pos] = num
